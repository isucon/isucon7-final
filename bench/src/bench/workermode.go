...
func runWorkerMode(tempDir, portalUrl string) {
	...
	for _, arg := range os.Args {
		if strings.HasPrefix(arg, "-remotes") || strings.HasPrefix(arg, "-output") {
	...
	for {
		job := getJobLoop()
		name := fmt.Sprintf("isucon7f-benchresult-%d-%d.json", time.Now().Unix(), job.ID)
		...
		tm := time.AfterFunc(120*time.Second, func() {
			defer cancel()
